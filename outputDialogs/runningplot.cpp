...
CurveData::CurveData(SignalData& paramSignalData):signalData(paramSignalData)
{
...
runningPlot::runningPlot(SignalData &paramSignalData, int minAxis, int maxAxis ,
                         QwtPlotCurve::CurveStyle curveStyle, QWidget *parent):
    QwtPlot( parent ),
    d_paintedPoints( 0 ),
    d_interval( 0.0, 20.0 ),
    d_timerId( -1 )
{
    ...
    setAxisScale( QwtPlot::yLeft, minAxis, maxAxis );
    ...
    d_curve->setStyle( curveStyle );
    if ( curveStyle == QwtPlotCurve::Dots)
        d_curve->setPen(canvas()->palette().color( QPalette::WindowText ), 4 );
    else
        d_curve->setPen( canvas()->palette().color( QPalette::WindowText ) );
    d_curve->setRenderHint( QwtPlotItem::RenderAntialiased, true );
    d_curve->setPaintAttribute( QwtPlotCurve::ClipPolygons, false );
    d_curve->setData( new CurveData(paramSignalData) );
    d_curve->attach( this );
...
runningPlot::~runningPlot()
{
    delete d_directPainter;
}

void runningPlot::start()
{
    d_clock.start();
    d_timerId = startTimer( 20 );
}

void runningPlot::replot()
{
...
void runningPlot::setIntervalLength( double interval )
{
...
void runningPlot::activateZoomer(bool activate)
{
    zoomer->setEnabled(activate);
}

void runningPlot::disActivateGraph(bool activate)
{
    activateZoomer(activate);
}

void runningPlot::updateCurve()
{
    CurveData *data = static_cast<CurveData *>( d_curve->data() );
    data->values().lock();
...
void runningPlot::incrementInterval()
{
...
void runningPlot::updateGraph( double elapsed )
{
    updateCurve();
    _elapsed = elapsed;
...
void runningPlot::resizeEvent( QResizeEvent *event )
{
    d_directPainter->reset();
    QwtPlot::resizeEvent( event );
}

void runningPlot::showEvent( QShowEvent * )
{
    replot();
}

bool runningPlot::eventFilter( QObject *object, QEvent *event )
{
    if ( object == canvas() &&
...
void runningPlot::timerEvent( QTimerEvent *event )
{
    if ( event->timerId() == d_timerId )
    {
        updateCurve();

        if ( _elapsed > d_interval.maxValue() )
            incrementInterval();
